#!/usr/bin/env python3
"""
Build a neofetch-style info card SVG to sit to the RIGHT of a portrait / banner:
colored key/value rows for work experience, tech stack, and highlights -- NOT
...
it feels like the panel is printing. STATIC=1 emits the frozen state for
Quick Look previews.
"""
import html
import os
...
ROWS = [
    ("host",),
    ("kv", "Now", ".NET Developer @ metamation"),
    ("kv", "Focus", "C# / WPF desktop apps"),
    ("kv", "Project", "Building Flux"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Lang", "C#, .NET"),
    ("kv", "Desktop", "WPF, MVVM"),
    ("kv", "Tools", "Visual Studio, Git, GitHub"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Clean, reliable software"),
    ("bul", "Always learning \u2014 one commit at a time"),
]
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (
            ...
            f'<line x1="{KEY_X + 12 + len(USER)*8 + 56}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
